...
- **AI-DERIVED CONTENT IS NEVER A COMMITMENT — FIRST-PERSON ONLY.** A summary is a claim about what
  ...
  one product.**
- **AND A FIELD A QUERY BUILT IS NOT A CAPTURE — NOTHING QUERY-SHAPED CLOSES A ROW.** A retrieval
  layer that composes a summary **per query** has answered your question instead of describing what
  is stored, **and the composed field is indistinguishable from a captured one at the point you read
  it** — same shape, same confidence, no marker. **The tell is that it moves when the query moves.**
  **Open the stored record before any close rests on it** (§10). This is the closure asymmetry at its
  worst: a wrongly-closed row costs the thing itself, silently, forever, and here the evidence for it
  was generated by the act of looking.
